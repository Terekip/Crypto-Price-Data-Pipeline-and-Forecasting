...
def fetch_crypto_data():
    ...
    for coin in coins:
        try:
           chart_data =cg.get_coin_market_chart_range_by_id(id=coin,                                                                          vs_currency='usd',                                                                 from_timestamp=from_ts,                                                            to_timestamp=to_ts                                                                 )
           coin_data = [{"timestamp": datetime.utcfromtimestamp(ts / 1000),                                "coin": rename_dict.get(coin, coin),  
                         "price": round(price, 2)
                    }
           ...
           if coin_data:
                ...
                df.to_sql('crypto_prices_raw', engine,
